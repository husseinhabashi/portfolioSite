...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isValidSignature) {
      await createAuditLog("signature_verification_failed", null, null, ip, userAgent, {
        inviteHash,
    ...
    if (!invite) {
      await createAuditLog("invite_not_found", null, null, ip, userAgent, { inviteHash })
      return NextResponse.json({ error: "Invite not found" }, { status: 404 })
    }

    // Check if invite is expired
    if (invite.expires_at && new Date(invite.expires_at) < new Date()) {
      await createAuditLog("invite_expired", invite.id, null, ip, userAgent, { inviteHash })
      return NextResponse.json({ error: "Invite has expired" }, { status: 401 })
    }

    // Check if invite is active
    if (!invite.is_active) {
      await createAuditLog("invite_inactive", invite.id, null, ip, userAgent, { inviteHash })
      return NextResponse.json({ error: "Invite is no longer active" }, { status: 401 })
    }

    await createAuditLog("invite_verified", invite.id, null, ip, userAgent, { inviteHash })
